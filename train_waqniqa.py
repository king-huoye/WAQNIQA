...
from models.waqniqa import WAQNIQA
from config import Config
from utils.process import RandCrop, ToTensor, Normalize, five_point_crop
from utils.process import split_dataset_kadid10k, split_dataset_koniq10k,split_dataset_pipal,split_dataset_tid2013,split_dataset_csiq,split_dataset_my
# 新加pipal
...
if __name__ == '__main__':
    ...
    config = Config({
        ...
        # PIPAL
        "train_dis_path": "/tmp/pycharm_project_535/PIPAL22/Train_Dis",
        "val_dis_path": "/tmp/pycharm_project_535/PIPAL22/Val_Dis",
        "pipal22_train_label": "/tmp/pycharm_project_535/data/PIPAL22/pipal22_train.txt",
        "pipal22_val_txt_label": "/tmp/pycharm_project_535/data/PIPAL22/pipal22_val.txt",

        # KADID-10K
        "kadid10k_path": "/tmp/pycharm_project_535/kadid10k/images/",
        "kadid10k_label": "/tmp/pycharm_project_535/data/kadid10k/kadid10k_label.txt",

        # KONIQ-10K
        "koniq10k_path": "/tmp/pycharm_project_535/1024x768",
        "koniq10k_label": "/tmp/pycharm_project_535/data/koniq10k/koniq10k_label.txt",

        #tid
        "tid_path":"/root/autodl-tmp/pycharm_project_535/tid2013/distorted_images",
        "tid_label":"/root/autodl-tmp/pycharm_project_535/data/tid2013/output.txt",

        #csiq
        "csiq_path":"/tmp/pycharm_project_535/CSIQ/image",
        "csiq_label":"/tmp/pycharm_project_535/data/CSIQ/csiq_label.txt",

        #自己的数据集
        'my_path':'/tmp/pycharm_project_535/Process-Dataset',
        'my_label':'/tmp/pycharm_project_535/data/Our/score.txt',
    ...
    if config.dataset_name == 'kadid10k':
        ...
        from data.tid2013.tid import TID
        train_name, val_name = split_dataset_tid2013(
            # 这里也要换
            txt_file_name=config.tid_label,
            split_seed=config.split_seed
        )
        dis_train_path = config.tid_path
        dis_val_path = config.tid_path
        label_train_path = config.tid_label
        label_val_path = config.tid_label
        Dataset = TID
        ...
        Dataset=CSIQ
    elif config.dataset_name=='our':
        # 改成我们的
        from data.Our.Ourdata import My
        train_name, val_name = split_dataset_my(
            txt_file_name=config.my_label,
            split_seed=config.split_seed
        )
        dis_train_path = config.my_path
        dis_val_path = config.my_path
        label_train_path = config.my_label
        label_val_path = config.my_label
        Dataset = My
    else:
        pass
    ...
    # model defination
    net = WAQNIQA(embed_dim=config.embed_dim, num_outputs=config.num_outputs, dim_mlp=config.dim_mlp,
